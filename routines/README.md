...
v2 移行済 (handoff `legacy-state-refs-cleanup`、2026-06-02): `lifecycle_stage` は drop 済 (`status` も同様)。全 routines は `state` 軸 (`active` / `completed` 等) に書き換え完了。Inbox は `list_recent` + クライアント側 `context_id IS NULL` フィルタ方式を採用 (`search_items` が `context_id IS NULL` をネイティブ表現できないため)。
